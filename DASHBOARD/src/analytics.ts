...
namespace RailSync.Analytics {
  ...
  /** A panel is a shell plus the scrolling body callers fill in. */
  ...
  export function renderReports(): void {
    ...
    Object.keys(m.byClass).forEach(function (k) {
      ...
      b2.appendChild(bar(k.charAt(0) + k.slice(1).toLowerCase() + ' (' + v.n + ')',
        v.pct, v.pct.toFixed(1) + '%', CLS_COLOR[k as TrainClass] || '#4a9eff'));
    });
    b2.appendChild(el('div', 'mini-note', 'Average delay by class: ' +
    ...
    D.corridors.forEach(function (c) {
      ...
      const det = el('div', 'mini-note',
        'probe ' + t.probe + ' @ ' + t.station + ' · ambient ' + t.amb + '°C · tₔ ' + t.dest + '°C · ' +
    ...
    b3.appendChild(el('div', 'mini-note',
      'Readings follow the division clock — play or fast-forward to see the day’s thermal window.'));
    ...
    b4.appendChild(el('div', 'mini-note',
      cleared + ' demand(s) cleared by approved blocks this shift, retiring ' + savedDays +
      ' overdue-days. ' + (D.queue.length - cleared) + ' still outstanding.'));
  ...
  export function renderLogs(): void {
    ...
    if (!S.log.length) {
      root.appendChild(el('div', 'empty',
        'No controller decisions yet. Approve, reject, shift or bundle a block and it is ' +
    ...
    S.log.forEach(function (L) {
      ...
      if (L.ref) {
        const p = S.proposals.filter(function (x) { return x.id === L.ref; })[0];
      ...
      if (L.ref) {
        ...
        it.addEventListener('click', function () { bus.emit('openProposal', L.ref as string); });
      }
      root.appendChild(it);
    });
  }
